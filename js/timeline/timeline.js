...
function renderTimeline(){
  ...
  S.textLayers.forEach(layer=>{
    ...
    const el=document.createElement('div');
    el.className='tl-textlayer'+(layer.id===S.selectedTextLayerId?' selected':'')+(layer.hidden?' tl-layer-hidden':'');
    el.style.left=left+'px';
    el.style.width=width+'px';
    el.dataset.textId=layer.id;
    el.textContent=layer.text||'Text';
    el.title=(layer.text||'Text')+(layer.hidden?' (hidden)':'');
  ...
  S.imageLayers.forEach(layer=>{
    ...
    const width=Math.max((safeEnd-safeStart)*zoom,20);
    const el=document.createElement('div');
    el.className='tl-imagelayer'+(layer.id===S.selectedImageLayerId?' selected':'')+(layer.hidden?' tl-layer-hidden':'');
    el.style.left=left+'px';
    el.style.width=width+'px';
    el.dataset.imgId=layer.id;
    const fname=(layer.path||'').split(/[/\\]/).pop()||'Image';
    el.textContent=fname;
    el.title=fname+(layer.hidden?' (hidden)':'');
