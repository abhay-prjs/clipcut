...
- **Right panel (inspector):** flat stacked sections replaced by tabs — `.insp-tabs`/`.insp-tab`/`.insp-panel`, switched via `switchInspTab(name)` (namespaced separately from `switchTab()` on purpose — they used to share `.panel-tab` and each call was wiping the other's active state)
  ...
  - **Text** tab: "Add Text" freeform text layers only (content textarea + per-layer font/weight/size/color/stroke/position, `js/ui/textlayers.js`) — its own outliner list (`#textLayerOutliner`) and always-visible "+ Text" button
  - **Overlay** tab: "+ Image" sticker/image overlays only (`js/ui/imagelayers.js`) — its own outliner list (`#imageLayerOutliner`), always-visible "+ Image" button, and the Asset Library grid (see below). Split out from a former shared "Text" tab specifically because nobody thought to open "Text" to add a sticker
