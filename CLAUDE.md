...
- `switchTab(name)` — switches left panel tab; matches `.panel-tab` elements by `data-tab` attribute (not position); calls `_loadWhisperConfigFromServer()` when switching to 'settings'. Tracks the last non-settings tab in `_lastPanelTab` (module-level `let` in ui.js) so `toggleSettingsTab()` knows what to return to
- `toggleSettingsTab()` — the topbar gear icon's handler (2026-07-28 session; it used to call `switchTab('settings')` directly, which had no way to close back out). Closes back to `_lastPanelTab` if Settings is already open; otherwise closes the AI chat panel (if open, via `toggleChatPanel()`) and opens Settings — the two side panels are meant to be mutually exclusive, not stacked
- `switchInspTab(name)` — switches right-panel inspector tab (Video/Captions/Layers); matches `.insp-tab`/`.insp-panel` by `data-insp` — deliberately separate from `switchTab()`, do not merge them or give inspector tabs the `.panel-tab` class. Also accepts `'text'`/`'overlay'` as a convenience (routes to the Layers tab + the matching sub-tab via `switchLayerSubTab()`) so existing call sites in textlayers.js/imagelayers.js didn't need to change
- `switchLayerSubTab(name)` — `'text'` | `'overlay'`, switches the Layers tab's own sub-tab bar (`.layer-subtab`/`.layer-subpanel`, matched by `data-layersub`); calls `refreshAssetLibrary()` on `'overlay'`
...
- **Asset Library ("working folder")** — every source video gets a sibling `<video>_assets/` folder (e.g. `myclip.mp4` → `myclip_assets/`, created next to it via `ensure_assets_folder()` in serve.py — a plain OS folder, not inside ClipCut's install dir, so dropping files in via Explorer works too). `addImageLayer()` now copies whatever the native picker returns into that folder via `import_asset()` (de-dupes filename collisions with a numeric suffix, never overwrites) before creating the layer, so a video's overlays stay self-contained next to it. The Overlay tab's Asset Library grid (`refreshAssetLibrary()`) lists that folder's images as click-to-add thumbnails (`list_assets()`) — clicking one calls `addImageLayerFromAsset(path)`, skipping the native picker entirely for anything already reused. Lazy-loaded on Overlay-tab switch (`switchInspTab('overlay')`) and re-fetched on clip switch (`selectClip()`), same lazy pattern as `_loadWhisperConfigFromServer()`. `_createImageLayerAtPlayhead(path,url)` is the shared layer-creation core both the picker flow and the asset-click flow call into
...
- **Topbar:** Logo (no dropdown) · icon-only `.action-cluster` pill (Import/Undo/Redo/Mark/Delete, tooltips via `title=`) · centered `#topbarClipName` readout (live clip name, synced by `syncTopbarClipName()` in `js/ui/ui.js` — called from `selectClip()`, `deleteClip()`, and `_applySnapshot()` undo/redo restore) · config status pills (icon-only ✓/✗, no text) · timecode · AI Chat icon (`#chatToggleBtn`, `toggleChatPanel()`) · Settings gear (`#settingsToggleBtn`, `toggleSettingsTab()`) · Export button (label kept). Chat and Settings are mutually exclusive and each toggles closed on a second click of its own icon — opening one closes the other (see `switchTab()`/`toggleSettingsTab()`/`toggleChatPanel()` above)
...
- **Right panel (inspector):** flat stacked sections replaced by tabs — `.insp-tabs`/`.insp-tab`/`.insp-panel`, switched via `switchInspTab(name)` (namespaced separately from `switchTab()` on purpose — they used to share `.panel-tab` and each call was wiping the other's active state). Only 3 top-level tabs (Video/Captions/Layers, 2026-07-28 session) — 5 flat tabs kept overflowing the panel width and clipping the last tab, so related content was grouped: Speed folded into Video, Text+Overlay folded into one "Layers" tab with its own sub-tab bar (`.layer-subtabs`/`.layer-subtab`/`.layer-subpanel`, `data-layersub`, switched via `switchLayerSubTab()`). `.insp-tabs` also gained mouse-wheel horizontal scroll (`_initInspTabsWheelScroll()` in ui.js) as a safety net if it ever overflows again
  - **Video** tab: Clip Info (wrapped in `.info-card`) + Flip + Color Filter + Snapshot Frame + Speed (Rate select, folded in from the former standalone Speed tab)
  - **Captions** tab: full caption style block (font/weight/layout/size/color/stroke/position)
  - **Layers** tab: sub-tab bar toggling between **Text** ("Add Text" freeform text layers, `js/ui/textlayers.js` — content textarea + per-layer font/weight/size/color/stroke/position, `#textLayerOutliner`, always-visible "+ Text" button) and **Overlay** ("+ Image" sticker/image overlays, `js/ui/imagelayers.js` — `#imageLayerOutliner`, always-visible "+ Image" button, Asset Library grid). `switchInspTab('text')`/`switchInspTab('overlay')` still work as shorthand for "open Layers on this sub-tab" — textlayers.js/imagelayers.js call sites were left unchanged
